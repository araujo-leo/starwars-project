<?php include __DIR__ . '/../partials/header.php'; ?>
    ...
    <style>
        ...
        .text-label { color: #888; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 1px; }
        .text-value { color: #fff; font-size: 1.1rem; font-weight: 500; }
        ...
        .nav-link.active { background-color: transparent !important; color: #FFE81F !important; border-bottom: 3px solid #FFE81F; }
        ...
        .data-chip:hover { background-color: #252525; border-color: #FFE81F; transform: translateY(-2px); color: #FFE81F; }
        .chip-avatar { width: 45px; height: 45px; border-radius: 50%; object-fit: cover; border: 2px solid #444; background-color: #000; }
        .chip-content { flex-grow: 1; min-width: 0; }
        .chip-name { font-weight: bold; font-size: 0.95rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .loading-text { display: inline-block; width: 80px; height: 12px; background-color: #333; border-radius: 4px; animation: pulse 1.5s infinite; }
    ...
    <div class="container mt-4 mb-5">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="/vehicles" class="text-decoration-none text-warning">Vehicles</a></li>
        ...
        <div id="vehicle-content" style="display: none;">
            ...
            <div class="row mb-5">
                <div class="col-md-4 text-center d-flex flex-column align-items-center">
                    <img id="vehicle-img" src="" class="rounded border border-3 border-warning shadow-lg mb-3" style="width: 100%; max-width: 300px; height: 200px; object-fit: cover;">
                    <h1 class="display-6 fw-bold text-white mb-0" id="vehicle-name"></h1>
                    <p class="text-muted mt-1" id="vehicle-model"></p>
                    <span class="badge bg-dark border border-secondary text-warning mt-1 px-3 py-2" id="vehicle-class"></span>
                </div>
                    ...
                    <div class="row g-3">
                        <div class="col-6 col-md-4">
                            <div class="stat-box">
                                ...
                                <div class="text-value"><span id="vehicle-cost"></span></div>
                            </div>
                        </div>
                        <div class="col-6 col-md-4">
                            <div class="stat-box">
                                <div class="text-label">Max Speed</div>
                                <div class="text-value"><span id="vehicle-speed"></span> km/h</div>
                            </div>
                        </div>
                        <div class="col-6 col-md-4">
                            <div class="stat-box">
                                <div class="text-label">Length</div>
                                <div class="text-value"><span id="vehicle-length"></span> m</div>
                            </div>
                        </div>
                        <div class="col-6 col-md-4">
                            <div class="stat-box">
                                <div class="text-label">Crew</div>
                                <div class="text-value" id="vehicle-crew"></div>
                            </div>
                        </div>
                        <div class="col-6 col-md-4">
                            <div class="stat-box">
                                <div class="text-label">Passengers</div>
                                <div class="text-value" id="vehicle-passengers"></div>
                            </div>
                        </div>
                        <div class="col-6 col-md-4">
                            <div class="stat-box">
                                <div class="text-label">Cargo Capacity</div>
                                <div class="text-value"><span id="vehicle-cargo"></span> kg</div>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="stat-box text-start px-4">
                                <div class="text-label">Manufacturer</div>
                                <div class="text-value" id="vehicle-manufacturer"></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div id="loader" class="text-center py-5">
            ...
            <p class="mt-2 text-muted">Starting engines...</p>
        </div>
    </div>

    <script>
        const vehicleId = <?= json_encode($data['id'] ?? null); ?>;
        ...
        // Mapeamento: Pilots na API são 'people'
        const resourceMap = {
            'films': 'films',
            'pilots': 'people'
        };
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                $('#loader').hide();
                $('#vehicle-content').fadeIn();

                // Cabeçalho
                $('#vehicle-name').text(v.name);
                $('#breadcrumb-name').text(v.name);
                $('#vehicle-model').text(v.model);
                $('#vehicle-class').text(v.vehicle_class);

                // Imagem (Retangular para veículos fica melhor)
                const initials = getInitials(v.name);
                $('#vehicle-img').attr('src', `https://placehold.co/400x250/000000/FFE81F/?text=${initials}&font=montserrat`);

                // Specs
                $('#vehicle-cost').text(v.cost_in_credits !== 'unknown' ? formatNumber(v.cost_in_credits) + ' credits' : 'Unknown');
                $('#vehicle-speed').text(v.max_atmosphering_speed);
                $('#vehicle-length').text(v.length);
                $('#vehicle-crew').text(v.crew);
                $('#vehicle-passengers').text(v.passengers);
                $('#vehicle-cargo').text(formatNumber(v.cargo_capacity));
                $('#vehicle-manufacturer').text(v.manufacturer);

                // Relacionados
                // 'pilots' na API retorna URLs de People
                renderRelatedList('#list-pilots', v.pilots, 'pilots', 'Plt');
                renderRelatedList('#list-films', v.films, 'films', 'Mov');

            }).fail(function() {
                $('#loader').html('<div class="alert alert-danger">Failed to retrieve vehicle data.</div>');
            });
        });
    </script>
